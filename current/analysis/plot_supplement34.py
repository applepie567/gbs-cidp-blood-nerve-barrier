...
def draw(results,out):
    ...
    font_path=Path('/usr/local/share/fonts/truetype/timesnewroman')
    for fp in font_path.glob('*.TTF'): font_manager.fontManager.addfont(str(fp))
    plt.rcParams.update({'font.family':available_serif(),'font.size':11,'text.color':'black','axes.labelcolor':'black','xtick.color':'black','ytick.color':'black','pdf.fonttype':42,'ps.fonttype':42,'svg.fonttype':'none'})
    ...
    save(fig,'Supplementary_Figure_3')
    ...
    save(fig,'Supplementary_Figure_4')
    print('Saved two figures in three formats')
