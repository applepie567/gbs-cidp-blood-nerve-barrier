...
def figure2():
    ...
    im=a.imshow(vals,cmap='RdBu_r',vmin=-4,vmax=4,aspect='auto')
    ...
    for i in range(7):
        for j in range(3):a.text(j,i,f'{vals[i,j]:.2f}'+('*' if fdr[i,j]<.05 else ''),ha='center',va='center',
                                ...
                                color='white' if abs(vals[i,j])>3.2 else 'black')
...
def figure5():
    ...
    cc.set_axis_off()
    rows=[('CDH4','Female-specific GWAS','OR 4.37',ORANGE),
